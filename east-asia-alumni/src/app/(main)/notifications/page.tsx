...
import { NotificationListA } from "@/components/notifications/NotificationListA"
...
export default async function NotificationsPage() {
  ...
  const actorIds = [...new Set((notificationsRaw ?? []).map((n: any) => n.actor_id).filter(Boolean))] as string[]
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <NotificationListA notifications={notifications} userId={user.id} />
    </div>
  )
}
